...
describe('Cart API (system)', () => {
    ...
    describe('POST /cart', () => {
        ...
        it('returns 401 without auth', async () => {
            const res = await request(app).post('/cart').send({ product_id: 2, quantity: 3 });
    ...
    describe('GET /cart', () => {
        it('returns cart items for authenticated user', async () => {
            const items = [{ product_id: 1, quantity: 2 }, { product_id: 3, quantity: 1 }];
            pool.query.mockResolvedValue({ rows: items });

            const res = await request(app).get('/cart').set('Authorization', `Bearer ${VERIFIED_TOKEN}`);
    ...
    describe('PATCH /cart/:product_id', () => {
        ...
        it('returns 401 without auth', async () => {
            const res = await request(app).patch('/cart/2').send({ quantity: 5 });
            expect(res.status).toBe(401);
        });
    });
